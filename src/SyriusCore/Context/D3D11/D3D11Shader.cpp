...
namespace Syrius{
    ...
    void D3D11Shader::bind() {
        SR_PRECONDITION(m_VertexShader != nullptr, "D3D11Shader failed to bind, invalid vertex shader");
        SR_PRECONDITION(m_PixelShader != nullptr, "D3D11Shader failed to bind, invalid pixel shader");

        m_Context->VSSetShader(m_VertexShader, nullptr, 0);
        m_Context->PSSetShader(m_PixelShader, nullptr, 0);
    }

    u64 D3D11Shader::getIdentifier() const {
        SR_LOG_WARNING("D3D11Shader", "has no unique identifier");
        return 0;
    }
}
