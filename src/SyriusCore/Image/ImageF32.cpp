...
namespace Syrius{
    ...
    Image(),
    m_Data(){
        SR_PRECONDITION(stbi_is_hdr(desc.fileName.string().c_str()), "[ImageF32]: Image: {} is not a HDR image", desc.fileName.string());
        ...
        if (!pData){
            SR_LOG_WARNING("ImageF32", "Image: {} failed to load", desc.fileName.string());
            return;
        }
    ...
    m_Data(desc.width * desc.height * getTextureChannelCount(desc.format), desc.defaultChannelValue){
        SR_PRECONDITION(desc.width > 0, "[ImageF32]: Image cannot be created with a width of 0");
        SR_PRECONDITION(desc.height > 0, "[ImageF32]: Image cannot be created with a height of 0");
        SR_PRECONDITION(getTextureDataType(desc.format) == SR_FLOAT32, "[ImageF32]: Image cannot be created with a format (given = {}) that is not of type float", desc.format)
        ...
        switch (desc.imgType){
            case SR_IMAGE_HDR: {
                if (!stbi_write_hdr(desc.fileName.string().c_str(), width, height, channelCount, &m_Data[0])){
                    SR_LOG_WARNING("ImageF32", "Image: {} failed to write", desc.fileName.string());
                }
                break;
            }
            default: {
                SR_LOG_WARNING("ImageF32", "Image: {} has an unsupported image type: {}", desc.fileName.string(), desc.imgType);
                break;
            }
        }
    }
    ...
    void ImageF32::resize(u32 width, u32 height) {
        ...
        if (!stbir_resize_float(&m_Data[0], m_Width, m_Height, 0, &resizedData[0], width, height, 0, channelCount)){
            SR_LOG_WARNING("ImageF32", "Image failed to resize");
            return;
        }
        m_Data = resizedData;
        m_Width = width;
